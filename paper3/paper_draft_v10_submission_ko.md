...
**배경:** 주요우울장애(MDD)는 고도로 다유전자성이지만, GWAS 발견을 치료제로 전환하는 것은 여전히 난제입니다. 우리는 PGC MDD2025 요약통계를 이용해 유전자 수준 매핑, 뇌 전사체 연관분석(TWAS), 그리고 **방향성 인지(direction-aware)** 약물 재배치 분석을 통합했습니다.

**방법:** PGC MDD2025의 유럽계·no-23andMe 하위집단(412,305례, 1,588,397 대조군; 유효 N≈1,152,650)을 분석했습니다. MAGMA로 유전자 수준 연관(18,734 유전자; 본페로니 2.67e-6), S-PrediXcan(GTEx v8 MASHR, 13 뇌조직)으로 TWAS를 수행했습니다. **TWAS 연관은 예측발현-형질 상관을 나타낼 뿐 멘델 무작위화(MR)와 동등하지 않으므로**, 인과성은 신중히 기술하고 colocalization/SMR을 필수 확증 단계로 명시합니다. DGIdb v5 약물-유전자 상호작용과 교차참조하되, **각 유전자의 위험 방향과 반대로 작용하는 약물만 우선순위화**하는 방향성 필터를 적용했습니다. 유전력은 LDSC로 추정 후 liability scale(K=0.15)로 변환했습니다.

**결과:** MAGMA는 358개 본페로니 유의 유전자(최상위 *DBH* 영역, P=1.43e-27)를 확인했습니다. S-PrediXcan은 314개 TWAS 유의 유전자(P<5e-6, 그중 275개 조직간 본페로니 통과)를 도출했습니다. *DRD2*가 최강 신호였으며(측좌핵 P=4.43e-28, **Z=−10.99: *DRD2* 예측발현이 낮을수록 MDD 위험 증가**), 관측척도 h²=0.0458±0.0016(**liability-scale h²≈0.084 (K=0.15, 사례비율 P≈0.21)**; λGC=1.85, 절편 1.091, 비율 7.6%—잔여 층화/혼재의 무시할 수 없는 기여, 나머지 92.4%는 다유전성)였습니다. 방향성 필터로 678개 원시 매칭이 **102개 방향성 일치 약물(승인 28개)** 로 축소됐습니다. 음의 *DRD2* Z와 일관되게, **D2 길항제가 아니라 도파민 작용제/부분작용제(브로모크립틴, 프라미펙솔, 로티고틴, 카베르골린, 아포모르핀, 아리피프라졸, 브렉스피프라졸)** 가 우선순위에 올랐습니다. **요약 기반 확증(BrainMeta 뇌 cis-eQTL 대상 SMR/HEIDI + COLOC)에서 _DRD2_는 colocalize되지 않았고**(COLOC PP4≈0; GWAS와 eQTL이 서로 다른 인과변이, PP3≈1.0), 반면 **_SLC12A5_(KCC2)·_FURIN_·_DCC_** 가 상위 후보로 도출됐으나 확증 수준은 상이합니다. 이 중 세 검정(SMR·HEIDI·COLOC)을 모두 통과한 것은 _SLC12A5_뿐이며(p_SMR=3.6×10⁻¹⁴, HEIDI p=0.20, PP4=0.996), _FURIN_은 강하게 공존(PP4=1.00)하나 HEIDI 미통과(p=0.002), _DCC_는 시사적 공존(PP4=0.72, 0.8 미만)이었습니다.

**결론:** 최대 규모 MDD GWAS·뇌 TWAS·방향성 인지 약물성·요약기반 확증을 통합한 결과, 가장 두드러진 TWAS 유전자 *DRD2*는 GWAS 신호와 **공존(colocalize)하지 않았고**, 대신 *SLC12A5*(KCC2)가 colocalization으로 확증된 인과 후보이며, *FURIN*(강한 공존이나 HEIDI 기각으로 잠정)·*DCC*(시사적, PP4=0.72)는 이차 후보였습니다. 따라서 **_DRD2_ 기반 도파민 약물은 탐색적 등급으로 강등**하고, 우선순위 약물 목록을 확증된 *SLC12A5*(KCC2)(및 COLOC/HEIDI 불일치 *FURIN*·시사적 *DCC*) 기준으로 재정렬합니다. 조직상관 보정은 ACAT로 완료했으며, 인과/치료 주장에 앞서 LiftOver 재정합(SNP>90%)·교차질환(조현병) mtCOJO 조건부 분석(향후), 독립 재현, 비유럽계 검증이 필요합니다.
...
반면 **_SLC12A5_(KCC2)** 는 세 방법 모두에서 견고하게 확증되어(p_SMR = 3.6×10⁻¹⁴, HEIDI p = 0.20, COLOC PP4 = 0.996) 가장 신뢰할 수 있는 인과 유전자로 지목되었다. **_FURIN_** 은 강하게 공존했으나(PP4 = 1.00; p_SMR = 1.8×10⁻¹²) HEIDI는 유의(p = 0.002)하여 국소 이질성(인근 다중 eQTL 등) 가능성을 시사하므로, 유의한 HEIDI는 단일 공유 인과변이를 기각하므로 PP4=1.00은 LD상 2차 eQTL을 반영할 가능성이 높다; 따라서 *FURIN*은 확증 lead가 아니라 COLOC/HEIDI 불일치 좌위로 fine-mapping(SuSiE-coloc) 대상으로 분류한다. **_DCC_** 는 시사적 근거를 보였다(p_SMR = 7.4×10⁻¹¹, HEIDI p = 0.28, PP4 = 0.72; PP4가 0.8 미만이라 "공존"이 아닌 "시사적"). 한편 *GPX1*·*NEGR1*은 SMR 유의했으나 공존하지 않아(PP4 < 0.20; 높은 PP3) 공유 인과보다 LD 연결 신호로 해석된다(표 2). colocalization을 BrainMeta probe가 있는 **TWAS 유의 유전자 208개 전체**로 확장하면 **30개(14%)만 공존(PP4>0.8; 표 S3)** 했으며(*SLC12A5*, *FURIN*, *GPR27*, *TRMT61A*, *SPPL3*, *DIAPH3*, *FADS1*, *SP4*, *CHRNA4*, *VRK2* 등), 대다수는 공존하지 않았다 — 이는 "대부분의 TWAS 연관이 공유 인과변이가 아닌 LD를 반영한다"는 문헌과 일치한다. 이 공존 집합이 약물성 평가로 이어지는 고신뢰 인과 유전자다. 30개 공존 유전자 중 사전 정의 알고리즘으로 핵심 표적을 선정했다: (i) PP4>0.8, (ii) SMR+HEIDI 보강, (iii) DGIdb의 방향성 일치·CNS 관련 약물 상호작용, (iv) MDD/신경정신 생물학적 근거. *SLC12A5*는 4가지 모두 충족, *FURIN*은 (i)(iii)(iv) 충족하나 HEIDI 단서. *DCC*는 **예외**로, PP4=0.72(0.8 미만, 30개 집합 비포함)이나 넷린/DCC 중피질 도파민 배선 근거로 시사적 수준임을 명시해 포함한다. 30개 공존 유전자 전체 순위표(표 S3)를 제공해 선택적 강조를 피한다. 이 결과는 유전자 수준 결론을 상당히 재보정한다: 도파민 *DRD2* 가설은 colocalization으로 뒷받침되지 않으며, 염소수송(*SLC12A5*/KCC2)·전구단백질 전환효소(*FURIN*)·축삭유도(*DCC*) 유전자가 공존 또는 시사 근거를 갖는 인과 후보로 부상한다.
...
도파민 축을 넘어, 방향성 일치 승인 후보로 **DDB1 표적 약물(탈리도마이드, 레날리도마이드, 포말리도마이드; *DDB1* Z=+4.7, 억제제 일치)** 과 **RHOA 차단제(퀴니딘; Z=+8.0)** 가 포함되었습니다. 우리는 이전에 과대 진술했던 **티로신 키나아제 억제제(TKI) 가설을 의도적으로 철회합니다**: 대부분의 TKI는 P-당단백질 유출 기질로 혈액뇌장벽 투과가 낮고, 그 *DRD2* 활성은 길항성이어서 추론된 치료 방향과 반대이므로 방향성 필터에서 제외됩니다. 마찬가지로 리튬이 5개 유전자(*ADCY2*, *ASIC2*, *CACNG2*, *CREB1*, *DRD2*)의 지지를 받은 것은 특정 기전 신호라기보다 DGIdb상의 비특이적 다약리(promiscuous polypharmacology)를 반영할 가능성이 높고(표준적(canonical) 리튬 표적인 GSK-3β·IMPase는 히트에 없었음), 이에 따라 가중치를 낮춥니다. *FURIN*(P=8.39e-16, Z=-8.05)은 강하게 공존하는(PP4=1.00) 주목할 만한 TWAS 우선 노드이나, HEIDI 불일치(p=0.002)로 단일 공유 인과변이가 보장되지 않아 검증된 표적이 아닌 **fine-mapping 대상 가설**로 보고합니다.
...
몇 가지 한계점은 주의를 요합니다. 첫째, 본 분석은 유럽계 혈통만을 대상으로 진행되어 일반화 가능성이 제한됩니다. 둘째, S-PrediXcan은 요약 통계량에 의존하므로 Steiger 방향성 검정이 불가능하나, 우리는 이를 보완하기 위해 BrainMeta 뇌 cis-eQTL 대상 SMR/HEIDI 및 COLOC를 수행했고(3.4절), 우선순위 유전자 중 *SLC12A5*·*FURIN*은 공존(PP4>0.8)했고 *DCC*는 시사적(PP4=0.72)이었으며, *DRD2*·*GPX1*·*NEGR1*은 공존하지 않았습니다 — 따라서 나머지 TWAS 유전자는 확정 인과가 아닌 후보입니다(단, coloc는 전체 cis-window 기반이라 locus-conditioned/fine-mapping 재분석으로 경계 사례(예: SMR/HEIDI 통과·coloc 미통과 *NEGR1*; coloc 통과·HEIDI 미통과 *FURIN*)를 정밀화할 수 있습니다). 추가로 (i) BrainMeta cis-eQTL(n=2,865)은 도구강도가 중간 수준이라 약한 eQTL 유전자는 SMR에서 놓칠 수 있고(위음성), (ii) PGC MDD2025 GWAS와 GTEx v8/BrainMeta eQTL 패널은 서로 비중복 독립 표본이므로 TWAS/SMR 연관이 표본중복으로 부풀려지지 않았습니다. 셋째, LDSC 절편 비율 7.6%(intercept=1.0907±0.0158)는 **무시할 수 없는 중간 수준의 잔여 층화/혼재**를 시사하며(잘 통제된 GWAS는 보통 <3%), 이전에 기술한 "최소한의 층화"는 정정합니다. 또한 관측척도 h²=0.0458은 liability scale(K=0.15=통상 가정하는 MDD 평생유병률, 분석 하위집단 사례비율 P≈0.21=412,305/2,000,702)에서 h²≈0.084로 변환되며(K=0.10–0.20에서 약 0.07–0.09 범위), 뇌 세포유형 특이적 LDSC-SEG 분석은 여전히 남아있습니다. 넷째, S-PrediXcan SNP 커버리지가 78%(rsID 매칭, LiftOver 미적용)로 통상 >90%에 못 미치나, **인과 결론은 S-PrediXcan이 아니라 전체 커버리지의 독립 데이터(BrainMeta)에 대한 SMR+COLOC에 근거하므로, 이 한계는 발견(위음성)에는 영향을 주되 확정된 결론을 편향하지는 않습니다**; LiftOver(hg19→hg38) 재정합은 향후 과제입니다. 다섯째, 교차질환 측면에서 *DRD2*가 조현병 좌위이나 이미 COLOC 비공존으로 제외됐고, mtCOJO(SCZ 조건부) 검증은 GCTA 가용 환경에서의 향후 과제로 둡니다. 마지막으로, DGIdb 주석은 출처 데이터베이스에 따라 품질 편차가 있어 일차/오프타겟 상호작용 혼동 가능성이 있습니다. 또한 방향성 필터는 예측 발현 수준이 수용체 활성/신호 방향에 단조 대응한다고 가정하나, 자가수용체·전후 시냅스 위치·보상적 상향조절로 역전될 수 있어(*DRD2*가 대표 사례) 표적-방향 일치는 우선순위 휴리스틱일 뿐 치료 방향의 증명은 아닙니다.

향후 연구는 형평성 있는 게놈 발견을 보장하기 위해 혈통 간(trans-ancestry) GWAS 메타 분석을 최우선으로 진행해야 합니다. 우리는 단일 세포 RNA-seq 뇌 주석을 활용하여 LDSC-SEG를 적용하고, 관찰된 *GPX1* 및 *BSN* 신호를 유도하는 특정 세포 유형을 규명할 계획입니다. 본 연구는 전체 TWAS 유전자에 대한 SMR/HEIDI·colocalization과 상관에 강건한 ACAT 다중검정을 이미 완료했습니다(3.4·3.3절). 남은 향후 과제(우선순위): (i) LiftOver(hg19→hg38) 재정합으로 S-PrediXcan 커버리지를 78%→>90%로 끌어올려 위음성 유전자 회복; (ii) mtCOJO/MTAG로 조현병/양극성 조건부 분석(GCTA 가용 환경 필요); (iii) 독립 코호트 재현(FinnGen, MVP); (iv) *DBH* 영역 fine-mapping; (v) locus-conditioned coloc로 경계 사례(*NEGR1* 등) 정밀화; (vi) **우선순위 유전자(확증 *SLC12A5*/KCC2, COLOC/HEIDI 불일치 *FURIN*, 시사적 *DCC*)** 와 그 방향성 일치 약물의 iPSC 유래 신경세포 실험 검증. 약물 재포지셔닝은 우선순위 유전자 기반 후보를 ClinicalTrials.gov와 교차 참조하여 위험이 완화된 기회를 식별합니다.
